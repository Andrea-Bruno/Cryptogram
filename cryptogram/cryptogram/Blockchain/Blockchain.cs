    ...
    public class Blockchain
    {
      ...
      public class Block
      {
        ...
        private byte[] BaseChecksum()
        {
          ...
          return Encoding.UTF8.GetBytes(PreviousChecksum + BaseComputation);
        }
        ...
        public bool AddToBlockchain(Blockchain Blockchain = null)
        {
          ...
          try
          {
            if ((!System.IO.Directory.Exists(Blockchain.Directory())))
              System.IO.Directory.CreateDirectory(Blockchain.Directory());
        ...
        private Dictionary<string, string> GetAllBodySignature()
        {
        ...
        {
          System.Security.Cryptography.HashAlgorithm hashType = new System.Security.Cryptography.SHA256Managed();
          byte[] hashBytes = hashType.ComputeHash(Encoding.UTF8.GetBytes(BodyRecord(false)));
          return hashBytes;
        }
        private string BodyRecord(bool WithSigatures)
        {
          ...
          if (Blockchain.Type != BlockchainType.LineOfText)
            DataPart = Converter.StringToBase64(_Data);
          else
            DataPart = _Data;
          ...
          return DataPart + Blockchain.FieldsSeparator + HexTimestamp + Signatures;
        }
      ...
      {
        return MapPath(Setup.Ambient.Repository + "/" + Group);
      }
      ...
      {
        return Directory(Group) + "/" + AbjustNameFile(Name) + ".bloks";
      }
      private string PathNameFile()
      {
        return Directory() + "/" + AbjustNameFile(Name) + ".bloks";
      }
      private static string AbjustNameFile(string Text)
      {
